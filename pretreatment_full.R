pretreat.full=function(params,serie,Int=TRUE,first=NULL,last=NULL,yrInterp=NULL,plotit=TRUE){

## This is the R version of the CharAnalysis CharPretreatment.m function
## originally developed by P. Higuera and available at https://sites.google.com/site/charanalysis.
## The original R code was written by Olivier Blarquez and is available at http://blarquez.com/public/code/pretreatment.R 
## and was slightly modified by Walter Finsinger to comply with the CPT_v12 function.
##
## The function requires a matrix as input with the following 6 columns: 
## - CmTop CmBot AgeTop AgeBot Volume 
## - A series from which to calculate accumulation rates
## Samples that have not been analysed should be given a sediment volume equal to 0
  ...
  if (nMissingValues > 0){  # if some levels were not sampled...
    ...
    for (i in 1:nGaps ){ # Fill in gaps via interpolation.
      ...
      if (length(yi[2:length(yi)-1]) != length(gapIn[i,1]:gapIn[i,2])){
        yi =yi[-2]      # Trim interpolated values if there are more 
...
## INTERPOLATE CHARCOAL DATA TO yrInterp INTERVALS:
...
## DERIVE CHARCOAL ACCUMULATION RATE:
acc=c()
...
if(plotit==TRUE){
# Values for plotting
age=c(matrix(c(ageTop, ageBot), 2, byrow = T)) 
accInit<-rep(acc,each=2)
ageInt=c(matrix(c(ybpI,ybpI+yrInterp), 2, byrow = T)) 
accInt<-rep(accI,each=2)
x.lim = rev(range(max(age), min(age)))
y.lim = c(0, max(na.omit(accInit)))

# plot
plot(age, accInit, type="l", col="grey", ylim=y.lim, xlim=x.lim, ylab="dat.i", xlab="Age cal BP")
...
## Return values 
output=structure(list(CmI=cmI, AgeI=ybpI, sedArI = sedAccI, countI=countI, volI=volI, ConcI=conI, ArI=accI))
# write.csv(output, "dat_i.csv")
return(output)
## Done
}
